function HalftoneVcViaErrorDiff2Auth()
...
nShares = 2;
nAbpsPerShare = 6; % number of ABPs in each share
nRowsCover = 768;
nColsCover = 768;
...
permMt = zeros(nRowsCover/Q, nColsCover/Q/2, 4); % each position is a permutation vector
...
for i = 1:ceil(nRowsCover/Q)
    for j = 1:ceil(nColsCover/Q/2)
        ...
        permMt(i,j,:) = randperm(4); % a random permuation of columns
...
for i = 1:ceil(nRowsCover/Q)
    for j = 1:ceil(nColsCover/Q/2)
        ...
        if watermarkImg(i,j) == 1 % the secret pixel is a white pixel, 填相同
            ...
            share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 2 ) = ...
                share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3), 1 );
            share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 2 ) = ...
                share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4), 1 );
        else                                                           % 填互补
            share((i-1)*Q+iIdxSip(1), (j-1)*Q+jIdxSip(1)+shift, 2) = ...
            ...
            share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 2 ) = ...
                255 - share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3), 1 );
            share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 2 ) = ...
                255 - share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4), 1 );
        end
    end
end
    ...
    for j = 1:ceil(nColsCover/Q/2)
        ...
        if secretImg(i,j+(ceil(nColsCover/Q/2))) == 1   % the secret pixel is a white pixel
            ...
            share((i-1)*Q+iIdxSip(2), (j-1)*Q+jIdxSip(2)+shift, 1 ) = ...
                share((i-1)*Q+iIdxSip(2), (j-1)*Q+jIdxSip(2)+shift, 2 );
            share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 1 ) = ...
                share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 2 );
            share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 1 ) = ...
                share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 2 );
        else
            share((i-1)*Q+iIdxSip(1), (j-1)*Q+jIdxSip(1)+shift, 1) = ...
            ...
            share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 1 ) = ...
                255 - share((i-1)*Q+iIdxSip(3), (j-1)*Q+jIdxSip(3)+shift, 2 );
            share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 1 ) = ...
                255 - share((i-1)*Q+iIdxSip(4), (j-1)*Q+jIdxSip(4)+shift, 2 );
        end
...
% 保存文件画图用
imwrite(share(:,:,1),'share14sips.bmp','bmp');
imwrite(share(:,:,2),'share24sips.bmp','bmp');
%----------------------------------------------------
...
save 'htImgs4sips.mat' htImg share;
...
% 保存图片画图用
imwrite(stacked,'stackedImg4sips.bmp','bmp');
imwrite(shiftStack,'shiftStack4sips.bmp','bmp');
